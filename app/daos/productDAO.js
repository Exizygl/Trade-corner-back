...
const searchPaginationCategory = async (search, tagIdList, page, limit, categoryIdList, orderType, orderValue, minimun, maximun) => await ProductModel.find({ "$or" : [{"title" : {
  "$in": search,
  $options:'i'}}, {"tagIdList" : {"$in": tagIdList}}
...
const searchPagination = async (search, tagIdList, page, limit,  orderType, orderValue, minimun, maximun) => await ProductModel.find({ $or : [{"title" : {
  "$regex": search[0],
  $options:'i'}}, {"tagIdList" : {"$in": tagIdList}}
